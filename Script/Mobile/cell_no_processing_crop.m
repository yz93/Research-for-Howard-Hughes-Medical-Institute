...
im1_path = 'Z:\Winter 2016\HHMI\RCP_1to1000\original_1_cropped.tif';
...
im2_path = 'Z:\Winter 2016\HHMI\RCP_1to1000\original_2_cropped.tif';
...
im3_path = 'Z:\Winter 2016\HHMI\RCP_1to1000\original_3_cropped.tif';
...
for_level = [1.1, 1.1, 1.0];
for_bareopen = [11, 11, 6];
crop_offset_x = [1330, 1292, 1426];
crop_offset_y = [1370, 1590, 899];

num_identified_labels = zeros(1, 3);

save_names = {'mobile_im1_data_crop_orig', 'mobile_im2_data_crop_orig', 'mobile_im3_data_crop_orig'};
for x = 1 : 3
...
%     end
    ...
    % in its PixelList matrix.
        BB = points(i).BoundingBox;
        BB(1) = BB(1) + crop_offset_x(x) - 1;
        BB(2) = BB(2) + crop_offset_y(x) - 1;
        for j = 1 : length(labels)
            if (rectint(BB, labels(j, :)) ~= 0)
                train_data (i,1) = 1;
                break;
                % so if there is duplicate centroids for one label, only
                % one is counted. An imperfect way to avoid overcounting 
                % candidates.
            end
        end
...
%===========Machine Learning============
load('mobile_im1_data_crop_orig');
mobile_im1_data = train_data;
load('mobile_im2_data_crop_orig');
mobile_im2_data = train_data;
load('mobile_im3_data_crop_orig');
...
for i = 1 : 3
    ...
    %true_bag_accu(i) = nnz(ens_predictions) / true_count(i);
